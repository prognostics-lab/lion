...
% Real parameters
fid = fopen("examples/air_effect/params_air.json", "r");
raw = fread(fid, inf);
str = char(raw');
fclose(fid);
...
fid = fopen("examples/air_effect/params_est_air.json", "r");
raw = fread(fid, inf);
str = char(raw');
fclose(fid);
...
writetable(out1_table, "examples/air_effect/data_air_real.csv");
...
out2_table = table(out2.tout, out2.simout.sf_temp.Data, out2.simout.air_temp.Data, ...
    ...
    VariableNames=["time", "sf_temp", "air_temp", "q_gen", "amb_temp"]);
writetable(out2_table, "examples/air_effect/data_air_est.csv");
