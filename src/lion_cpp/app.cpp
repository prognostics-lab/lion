...
App::App(AppConfig *conf, AppParams *params) {
  ...
  lion_status_t ret = lion_app_new(conf->get_handle(), params->get_handle(), handle);
...
Status App::step(double power, double amb_temp) { return static_cast<Status>(lion_app_step(handle, power, amb_temp)); }

Status App::run(std::vector<double> const &power, std::vector<double> const &amb_temp) {
