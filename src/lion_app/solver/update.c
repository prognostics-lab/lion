...
lion_status_t lion_slv_update(lion_app_t *app) {
  ...
  app->state.generated_heat = lion_generated_heat(
      app->state.current, app->state.internal_temperature,
      app->state.internal_resistance, app->state.ehc, app->params);
  app->state.surface_temperature =
      lion_surface_temperature(app->state.internal_temperature,
                               app->state.ambient_temperature, app->params);
