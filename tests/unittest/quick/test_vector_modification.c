...
lion_status_t test_modification_push1(lion_sim_t *sim) {
  lion_vector_t vec;
  ...
  uint32_t val = 20;
  LION_CALL(lion_vector_push(sim, &vec, &val), "Failed pushing element");
  ...
  LION_ASSERT_FAILS(lion_vector_push(sim, &vec, NULL));
  log_debug("Checking that the failed push does not modify the vector");
  LION_ASSERT_EQI(vec.len, 9);
  LION_ASSERT_EQI(vec.capacity, 16);

  log_debug("Checking that the push hsimened fine");
  uint32_t contained_val;
  LION_CALL(lion_vector_get(sim, &vec, 8, &contained_val), "Failed getting vec[8]");
...
lion_status_t test_modification_push2(lion_sim_t *sim) {
  lion_vector_t vec;
  size_t        capacity = 8;

  log_debug("Creating vector");
  LION_CALL(lion_vector_with_capacity(sim, capacity, sizeof(uint32_t), &vec), "Failed creating vector");
  ...
  log_debug("Checking that the push hsimened fine");
  uint32_t contained_val;
  LION_CALL(lion_vector_get(sim, &vec, 0, &contained_val), "Failed getting vec[0]");
  LION_ASSERT_EQI(contained_val, val);
...
lion_status_t test_modification_extend_array(lion_sim_t *sim) {
  lion_vector_t vec;
  uint32_t      data[] = {0, 1, 2, 3, 4, 5, 6, 7};
  size_t        len    = 8;
  ...
  size_t   values_len = 3;
  LION_CALL(lion_vector_extend_array(sim, &vec, values, values_len), "Failed extending array");
  ...
  log_debug("Checking border scenario");
  LION_ASSERT_FAILS(lion_vector_extend_array(sim, &vec, NULL, 10010));
  log_debug("Checking that the failed extend does not modify the vector");
  LION_ASSERT_EQI(vec.len, 11);
  LION_ASSERT_EQI(vec.capacity, 11);

  log_debug("Checking that the extend hsimened fine");
  uint32_t contained_val;
  for (uint32_t i = 0; i < len; i++) {
    LION_VCALL(lion_vector_get(sim, &vec, i, &contained_val), "Failed getting vec[%d]", i);
    logi_debug("Found %u", contained_val);
    LION_ASSERT_EQI(contained_val, data[i]);
  }
  for (uint32_t i = 0; i < values_len; i++) {
    LION_VCALL(lion_vector_get(sim, &vec, i + len, &contained_val), "Failed getting vec[%d]", i + len);
    logi_debug("Found %u", contained_val);
